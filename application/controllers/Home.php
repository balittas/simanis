	...
	{		
		...
		public function index()
		{
			...
			$data['budidaya'] = $this->m_serat->selectSerat();
                  ...
                  $data['pengunjungTotal'] = $this->m_data->getTotalVisitor();
                  $data['pengunjungHariIni'] = $this->m_data->getTotalToday($tanggal); 
                  $data['pengunjungBulanIni'] = $this->m_data->getTotalByMonth($bulanIni); 

                  // $this->load->view('HeaderHome');
                  $this->load->view('HalamanUtamaPemanis', $data);
